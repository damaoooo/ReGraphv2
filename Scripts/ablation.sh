...
python evaluation.py /path/to/rell/IR/dataset-1/train_final_set/train_dataset_pool  /path/to/rell/IR/dataset-1/test_final_set/train_positive_map.pkl ${model_dir} --max-length 4096 -e ./db1_${graph_tag}_test.pkl.npy -b 16 --gpu-batch-size 256 "${eval_graph_flags[@]}" --bf16 2>&1 | tee >(sed -r 's/\x1B\[[0-9;?]*[ -\/]*[@-~]//g' | tr '\r' '\n' >> "${output_file}")
...
python evaluation.py /path/to/rell/IR/Dataset-2/db2_final_set/train_dataset_pool  /path/to/rell/IR/Dataset-2/db2_final_set/train_positive_map.pkl ${model_dir} --max-length 4096 -e ./db2_${graph_tag}_test.pkl.npy -b 16 --gpu-batch-size 256 "${eval_graph_flags[@]}" --bf16 2>&1 | tee >(sed -r 's/\x1B\[[0-9;?]*[ -\/]*[@-~]//g' | tr '\r' '\n' >> "${output_file}")
