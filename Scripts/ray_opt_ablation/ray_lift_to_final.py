#!/usr/bin/env python3
"""Ray driver for the combined binary -> lift -> final_set pipeline."""
...
DEFAULT_INPUT_PATH = Path("/scratch/zhoul0e/Dataset-1")
DEFAULT_SMOKE_INPUT_PATH = Path("/scratch/zhoul0e/Dataset-smoketest-lift")
DEFAULT_CACHE_ROOT = Path("/scratch/zhoul0e/regraph_cache")
DEFAULT_RELL_PYTHON = Path("/scratch/zhoul0e/miniconda3/envs/ReLL/bin/python")
DEFAULT_IDADIR = Path("/scratch/zhoul0e/ida-pro-9.3")
DEFAULT_IDA_HOME = Path("/scratch/zhoul0e")
...
def prepare_output_root(output_root: Path, resume: bool, force_clean: bool) -> None:
    ...
    ensure_dir(output_root / ".ray_lift2final_state")
...
def discover_source_binaries(input_path: Path, ll_root: Path) -> list[dict[str, Any]]:
    """Scan input directory for binary files to lift."""
    items: list[dict[str, Any]] = []
    for split, root in split_roots(input_path):
        for binary in sorted(root.rglob("*"), key=lambda p: p.relative_to(input_path).as_posix()):
            if not is_candidate_binary(binary):
                continue
            relative = binary.relative_to(input_path)
            output_ll = (ll_root / relative).with_suffix(relative.suffix + ".ll")
            ll_done_marker = ll_root / ".lift_done" / relative.with_suffix(relative.suffix + ".ll.done")
            items.append(
                {
                    "split": split,
                    "binary_path": str(binary.resolve()),
                    "relative_path": str(relative),
                    "output_ll": str(output_ll.resolve()),
                    "marker": str(ll_done_marker.resolve()),
                }
            )
    items = sorted(items, key=lambda item: stable_hash_key(item["relative_path"]))
    rng = random.Random(stable_hash_key("lift_order_v1"))
    rng.shuffle(items)
    return items


def discover_source_ll_files(ll_root: Path, output_root: Path, opt_level: str) -> list[dict[str, Any]]:
    """Discover lifted .ll files for task2 input."""
    items: list[dict[str, Any]] = []
    bc_root = output_root / "bc"
    for split, root in split_roots(ll_root):
        for source_ll in root.rglob("*.ll"):
            ...
            if any(part.endswith("_functions") for part in source_ll.parts):
                continue
            relative = source_ll.relative_to(ll_root)
            output_bc = (bc_root / relative).with_suffix(".bc")
            marker = output_root / ".ray_lift2final_state" / "task2_done" / relative.with_suffix(".bc.done")
            items.append(
                {
                    "split": split,
...
@ray.remote
def lift_chunk(chunk_id: str, items: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
    """Ray task: lift binaries to .ll using IDA Pro via native ReLL Python."""
    import subprocess as sp

    start = time.time()
    rell_python = context["repll_python"]
    ida2llvm = context["ida2llvm"]
    ida_dir = context["ida_dir"]
    ida_home = context["ida_home"]
    timeout = int(context.get("lift_timeout_seconds", 28800))
    process_timeout = int(context.get("lift_process_timeout_seconds", timeout))
    ...
    env["HOME"] = ida_home
    env["IDADIR"] = ida_dir
    env["REGRAPH_IDA_HOME"] = ida_home

    for item in items:
        output_ll = Path(item["output_ll"])
        marker = Path(item["marker"])
        if resume and output_ll.exists() and output_ll.stat().st_size > 0 and marker.exists():
            skipped.append(item)
            continue

        ensure_dir(output_ll.parent)
        ensure_dir(marker.parent)
        if marker.exists():
            marker.unlink()

        cmd = [rell_python, ida2llvm, "-f", item["binary_path"], "-o", str(output_ll)]

        # Wait for free memory before launching IDA to avoid swap thrashing
        mem_wait_sec = 0
        while True:
            try:
                with open("/proc/meminfo") as f:
                    meminfo = f.read()
                avail_match = [l for l in meminfo.splitlines() if l.startswith("MemAvailable:")]
                if avail_match:
                    avail_kb = int(avail_match[0].split()[1])
                    if avail_kb > 16_000_000:  # > 16GB free (z3 needs ~16GB peak)
                        break
            except Exception:
                break
            if mem_wait_sec >= 600:  # give up waiting after 10 min
                break
            time.sleep(2 + random.random() * 6)  # 2-8s jitter to avoid thundering herd
            mem_wait_sec += 5
        if mem_wait_sec > 0:
            print(f"[{chunk_id}] waited {mem_wait_sec}s for free memory before lifting {item['binary_path']}", flush=True)

        try:
            result = sp.run(cmd, capture_output=True, text=True, env=env, timeout=process_timeout)
            if result.returncode == 0 and output_ll.exists() and output_ll.stat().st_size > 0:
                marker.write_text(json.dumps({"binary": item["binary_path"]}) + "\n")
                successes.append(item)
            else:
                if output_ll.exists():
                    output_ll.unlink(missing_ok=True)
                error_msg = (result.stderr or result.stdout or f"ida2llvm returncode={result.returncode}")[:4000]
                if result.returncode < 0:
                    oom_retries.append({"item": item, "error": f"[OOM retry] signal={-result.returncode}: {error_msg}"})
                else:
                    failures.append({"item": item, "error": error_msg})
        except sp.TimeoutExpired as exc:
            if output_ll.exists():
                output_ll.unlink(missing_ok=True)
            stderr = exc.stderr if isinstance(exc.stderr, str) else (exc.stderr or b"").decode(errors="replace")
            oom_retries.append({"item": item, "error": f"[OOM retry] IDA stuck/timeout after {process_timeout}s\n{stderr}"[:4000]})
        except Exception as exc:
            if output_ll.exists():
                output_ll.unlink(missing_ok=True)
            failures.append({"item": item, "error": f"IDA lift exception: {exc}"[:4000]})
...
# ---------------------------------------------------------------------------
# Ray stage runner (shared across lift and task2)
# ---------------------------------------------------------------------------
...
def run_ray_stage(
    ...
    def emit_summary(reason: str, force: bool = False) -> None:
        ...
        if not force and now < next_summary:
            return
        completed_items = len(successes) + len(failures) + len(skipped) + len(oom_retries)
        ...
        logger.info(
            ...
            f"success={len(successes)} failed={len(failures)} oom={len(oom_retries)} skipped={len(skipped)} "
    ...
    write_jsonl(output_root / "manifests" / f"{stage}_oom_retries.jsonl", oom_retries)
    logger.write_failures(stage, failures)
    emit_summary("complete", force=True)
    logger.info(
        f"stage={stage} complete success={len(successes)} skipped={len(skipped)} "
        f"failed={len(failures)} oom_retries={len(oom_retries)}"
    )
    if failures:
    ...
    return successes, failures, skipped, oom_retries
...
# ---------------------------------------------------------------------------
# Utility functions (reused from fused pipeline)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ray combined binary -> lift -> final_set pipeline")
    ...
    parser.add_argument("--repll-python", default=os.environ.get("REGRAPH_RELL_PYTHON", str(DEFAULT_RELL_PYTHON)))
    parser.add_argument("--ida-dir", default=os.environ.get("REGRAPH_IDADIR", str(DEFAULT_IDADIR)))
    parser.add_argument("--ida-home", default=os.environ.get("REGRAPH_IDA_HOME", str(DEFAULT_IDA_HOME)))
    parser.add_argument("--lift-chunk-size", type=int, default=1)
    parser.add_argument("--lift-timeout-seconds", type=int, default=28800)
    parser.add_argument("--lift-process-timeout-seconds", type=int, default=3600,
                        help="Per-file IDA subprocess timeout; stuck files go to OOM retry (default 1h)")
    ...
    parser.add_argument("--final-local-root", default=os.environ.get("REGRAPH_FINAL_LOCAL_ROOT", ""))
...
def main() -> int:
    ...
    if args.smoke:
        input_path = DEFAULT_SMOKE_INPUT_PATH
    input_path = input_path.resolve()
    if not input_path.exists():
        raise SystemExit(f"Input path does not exist: {input_path}")

    output_root = Path(args.output_path) if args.output_path else input_path.with_name(f"{input_path.name}-{opt_state_token(opt_level)}-lift2final")
    ...
    rell_python = args.repll_python
    ida2llvm_path = str(Path(repo_root) / "Scripts" / "ida2llvm.py")
    ...
    try:
        ...
        logger.info(f"ida2llvm={ida2llvm_path}")
        logger.info(f"ida_dir={ida_dir}")
        ...
        # ---- Stage: Lift (binaries -> .ll) ----
        lift_items = discover_source_binaries(input_path, ll_root)
        if not lift_items:
            logger.info("stage=lift no binary files found, checking for existing .ll files")
        else:
            lift_context = {
                **base_context,
                "repll_python": rell_python,
                "ida2llvm": ida2llvm_path,
                "ida_dir": ida_dir,
                "ida_home": args.ida_home,
                "lift_timeout_seconds": args.lift_timeout_seconds,
                "lift_process_timeout_seconds": args.lift_process_timeout_seconds,
            }
            lift_chunk_size = choose_chunk_size(len(lift_items), cpus, args.lift_chunk_size, 1)
            logger.info(f"lift_chunk_size={lift_chunk_size}")
            with Progress(
                SpinnerColumn(),
                ...
                lift_success, lift_fail, lift_skip, lift_oom = run_ray_stage(
                    "lift",
                    lift_items,
                    lift_chunk,
                    lift_chunk_size,
            ...
            # Retry OOM failures until none left or no progress
            retry_round = 0
            while lift_oom:
                retry_round += 1
                retry_items = [f["item"] for f in lift_oom]
                rng = random.Random(stable_hash_key(f"lift_retry_{retry_round}"))
                rng.shuffle(retry_items)
                logger.info(
                    f"stage=lift_retry round={retry_round} items={len(retry_items)} "
                    f"rescheduling OOM-killed files across cluster"
                )
                retry_chunk_size = choose_chunk_size(len(retry_items), cpus, args.lift_chunk_size, 1)
                with Progress(
                    SpinnerColumn(),
                    TextColumn("[progress.description]{task.description}"),
                    BarColumn(),
                    ...
                    retry_success, retry_fail, retry_skip, retry_oom = run_ray_stage(
                        f"lift_retry_{retry_round}",
                        retry_items,
                        lift_chunk,
                        retry_chunk_size,
                        lift_context,
                ...
                lift_success.extend(retry_success)
                lift_fail.extend(retry_fail)
                lift_skip.extend(retry_skip)
                if not retry_success and retry_oom:
                    logger.info(
                        f"stage=lift_retry round={retry_round} no progress: "
                        f"all {len(retry_oom)} items still OOM-killed, giving up retries"
                    )
                    for f in retry_oom:
                        f["error"] = f["error"].replace("[OOM retry]", f"[OOM exhausted after {retry_round} retries]")
                    lift_fail.extend(retry_oom)
                    lift_oom = []
                else:
                    lift_oom = retry_oom
                    logger.info(
                        f"stage=lift_retry round={retry_round} "
                        f"success={len(retry_success)} failed={len(retry_fail)} oom_left={len(lift_oom)}"
                    )

            if lift_fail:
                usable_ll = sum(1 for _ in ll_root.rglob("*.ll"))
                logger.info(
                    f"stage=lift continuing_after_failures failed={len(lift_fail)} "
                    f"usable_ll_files={usable_ll}"
                )
                if usable_ll == 0:
                    logger.info("pipeline finished because lift produced no usable .ll files")
                    return 1

        # ---- Stage: Task2 (.ll -> .bc) ----
        source_items = discover_source_ll_files(ll_root, output_root, opt_level)
        if not source_items:
            raise SystemExit(f"No .ll files found under {ll_root} — lift may have produced nothing")
        ...
        with Progress(
            ...
            task2_success, task2_fail, task2_skip, _task2_oom = run_ray_stage(
                "task2",
                source_items,
                task2_chunk,
                task2_chunk_size,
        ...
        # ---- Stage: DataProcess (parquet -> HF dataset) ----
        dataprocess_command = [
            sys.executable,
            "-m",
            "DataProcess.cli",
