...
log_file = os.path.join(os.path.dirname(__file__), "ida2llvm.log")
logging.basicConfig(filename=log_file, level=logging.DEBUG, filemode='a',  # 'w' for overwrite, 'a' for append
...
def lift_tif(tif: ida_typeinf.tinfo_t, width = -1) -> ir.Type:
    ...
    if tif.is_func():
        ...
        # Handle _UNKNOWN* specifically.  If the child type isn't known, treat as void*.
        if str(child_tif) == "_UNKNOWN":
            logging.debug("Encountered _UNKNOWN type.  Returning i[width].")
            return ir.IntType(ptrsize).as_pointer()
        else:
            llvm_type = lift_tif(child_tif)
